...
export const MatchQuestionView: React.FC<MatchQuestionViewProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs hover:shadow-md transition-all overflow-hidden mb-6 p-5 sm:p-6" id={`match-card-${question.id}`}>
      {/* Header Meta */}
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4 pb-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          <span className="bg-purple-600 text-white text-xs font-black px-2.5 py-1 rounded-md tracking-wide">
            Q{index + 1}
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-purple-700 bg-purple-50 px-2 py-0.5 rounded border border-purple-200">
      ...
      <div className="space-y-1 mb-5">
        ...
        {(displayMode === 'tamil' || displayMode === 'bilingual') && question.question_ta && (
          <p className="text-sm sm:text-base font-semibold text-slate-700 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80">
          <div className="flex items-center justify-between pb-2 mb-3 border-b border-slate-200">
            ...
            <span className="text-[10px] font-semibold text-slate-400">{leftItems.length} Items</span>
          </div>
          <div className="space-y-2.5">
            {leftItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 bg-white p-2.5 rounded-lg border border-slate-200/90 shadow-2xs">
                <span className="w-6 h-6 rounded-md bg-purple-100 text-purple-800 text-xs font-extrabold flex items-center justify-center shrink-0">
                  {item.id}
                </span>
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80">
          <div className="flex items-center justify-between pb-2 mb-3 border-b border-slate-200">
            ...
            <span className="text-[10px] font-semibold text-slate-400">{rightItems.length} Items</span>
          </div>
          <div className="space-y-2.5">
            {rightItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 bg-white p-2.5 rounded-lg border border-slate-200/90 shadow-2xs">
                <span className="w-6 h-6 rounded-md bg-indigo-100 text-indigo-800 text-xs font-extrabold flex items-center justify-center shrink-0">
                  {item.id}
                </span>
                <div className="min-w-0 flex-1">
                  {(displayMode === 'english' || displayMode === 'bilingual') && (
      ...
      <div className="bg-purple-50/50 border border-purple-200/80 p-4 sm:p-5 rounded-xl mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {leftItems.map((leftItem) => {
            ...
            return (
              <div key={leftItem.id} className="flex items-center gap-2 bg-white p-2.5 rounded-lg border border-slate-200 shadow-2xs">
                <span className="w-7 h-7 rounded-md bg-purple-600 text-white font-black text-xs flex items-center justify-center shrink-0">
                  {leftItem.id}
                </span>

                <span className="text-xs font-semibold text-slate-700 truncate max-w-[100px] sm:max-w-[120px]">
                  {leftItem.text_en}
                </span>
                ...
                <select
                  ...
                  className={`flex-1 text-xs sm:text-sm py-1.5 px-2 rounded-md border font-bold bg-white text-slate-800 transition-colors focus:outline-hidden ${selectorBorder} ${
      ...
      {showSolution && (
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 text-xs sm:text-sm space-y-3 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {leftItems.map((lItem) => {
              ...
              return (
                <div key={lItem.id} className="bg-white p-2.5 rounded-lg border border-emerald-200 text-emerald-950 flex items-center gap-2">
          ...
          {(question.explanation_en || question.explanation_ta) && (
            <div className="pt-2 border-t border-slate-200 text-slate-600">
              {question.explanation_en && <p>{question.explanation_en}</p>}
              {question.explanation_ta && <p className="font-sans text-xs text-slate-500 mt-1">{question.explanation_ta}</p>}
            </div>
          )}
        </div>
      )}
